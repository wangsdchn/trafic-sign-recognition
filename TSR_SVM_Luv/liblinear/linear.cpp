...
double predict_values(const struct model *model_, const struct feature_node *x, double *dec_values,double& score)
{
	int idx;
	int n;
	if(model_->bias>=0)
...
double predict(const model *model_, const feature_node *x)
{
	...
	double score;
	double label=predict_values(model_, x, dec_values,score);
	free(dec_values);
	return label;
}
...
double predict_probability(const struct model *model_, const struct feature_node *x, double* prob_estimates)
{
	if(check_probability_model(model_))
	{
		...
		if(nr_class==2)
			...
			nr_w = nr_class;
		double score;
		double label = predict_values(model_, x, prob_estimates, score);
		for(i=0;i<nr_w;i++)
