...
bool sort1(Rect left, Rect right){
...
//	imwrite("../Luv.png", LuvImg);
	...
	for (int i = 0; i < nr; i++)
	{
		...
		for (int j = 0; j < nc; j = j + channels)
		{
			...
			if (red>MAX(blue, yellow))
				p_gray[j / channels] = lut[saturate_cast<int>(255 * red)];	//saturate_cast<uchar>
			else if (blue>MAX(red, yellow))
				p_gray[j / channels] = lut[saturate_cast<int>(255 * blue)];
			else
				p_gray[j / channels] = lut[saturate_cast<int>(255 * yellow)];
		}
	}
	cout << (getTickCount() - time0) / getTickFrequency() << endl;
	...
	vector<Rect> boxs;
	MSER mserExtractor(4, 80, 3600, 0.35, 0.2, 200, 1.01, 0.003, 5);
...
//		rectangle(src, boundRect[i], Scalar(0, 255, 0));
	...
	sort(goodBoundRect.begin(), goodBoundRect.end(), sort1);
	float thresh = 1000.0;
	float distance;
